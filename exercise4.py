...
# So the first time a paperboy makes a delivery, the quota is 50. The next time, the quote is 50 plus half the number of papers that the paperboy has delivered in the past.
...
class Paperboy:
	"""docstring for Paperboy"""
	def __init__(self, name,earnings,experience):
			self.name = name
			self.EXP = experience
			self.earnings = earnings

			def __str__(self):
  				print("Hello I work at newspaper serivces and my name is {}. I've delivered {} papers and I've earned {} so far!".format(self.name, self.experience, self.earnings))


  				def quota(self):
  					if self.EXP <= 0:
  						work_quota = 50
  					else: 50 + int(self.experience / 2)
  					return quota

  				def paper_route(self, start_address, end_address):
  					num_houses = end_address - start_address
  					total = 0
        		if number_houses <= self.quota():
        		    total = num_houses * 0.25
        		elif num_houses > self.quota():
         		   total = (self.quota() * 0.25) + ((num_houses - self.quota()) * 0.5)
        		elif num_houses < quota:
            		total -= 2
...
Paperboy1 = Paperboy('Josh', 1, 50)
Paperboy1.paper_route(3,45)
print(Paperboy1.work_done())